    ...
    <div className="w-full drop-shadow-md">
      ...
      <div className={`flex justify-between items-center border-b pb-3 mb-6 px-2 ${isDark ? "border-white/15" : "border-gray-900/10"}`}>
        ...
        <button
          ...
          className={`text-[11px] font-medium uppercase tracking-[0.15em] cursor-pointer transition-colors ${
      ...
      <div className="flex gap-2 md:gap-4 overflow-x-auto hide-scrollbar pb-4 px-2">
        {days.map((day, index) => (
          <div
            key={index}
            className={`flex flex-col items-center justify-between text-center min-w-[100px] md:min-w-[120px] flex-shrink-0 py-4 px-3 rounded-xl transition-all cursor-default group ${
          ...
          >
            ...
            <div className={`flex flex-col text-[11px] font-medium tracking-wide mb-4 gap-0.5 ${isDark ? "text-white/70" : "text-gray-600"}`}>
              <span>
                min <span className={isDark ? "text-white/90" : "text-gray-900"}>{formatTemp(day.minTemp)}</span>
              </span>
              <span>
                max <span className={isDark ? "text-white/90" : "text-gray-900"}>{formatTemp(day.maxTemp)}</span>
              </span>
            </div>
